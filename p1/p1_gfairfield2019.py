...
class Checker(TwoPlayerGame):
    ...
    def make_move(self, pos):
        ...
        ------
        """
        if self.current_player == 1: self.players[1].pos = self.white_pieces
        else: self.players[0].pos = self.black_pieces
    ...
    def lose(self):
        ...
        """
        """
        if self.current_player == 1: return self.black_pieces in self.white_territory
        else: return self.white_pieces in self.black_territory
        """
        if self.current_player == 1:
            for val in self.black_pieces:
                if val in self.white_territory:
                    return True
                else: return False
        else:
            for val in self.white_pieces:
                if val in self.black_territory:
                    return True
                else: return False
